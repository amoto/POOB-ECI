...
public class Evolution
{
    ...
    public Evolution(String organism,String[] fossils)
    {
        ...
        if(isOk){
            ...
            if(actual.getLen()>(MAXH/sz)-1 || nFosilesLibres>(Math.ceil(MAXV/3.28)-2*sz)/sz+1) {
                JOptionPane.showMessageDialog(null,"No se va a hacer visible");
            }
        }
    }
    ...
    {
        boolean isP = true;
        if(nFosilesLibres < (Math.ceil(MAXV/3.28)-2*sz)/sz+1 && actual.getLen() < (MAXH/sz)-1){
            if(nLineas>0  && lineas.get(nLineas-1).getEnd()+5 < MAXH){
                boolean fosilesVisibles = true;
                for(int i = 0; i < nLineas && fosilesVisibles; i++){
                    fosilesVisibles = lineas.get(i).getLength()*(sz+10) < MAXV-(Math.ceil(MAXV/3.28)+sz);
                }
                isP = fosilesVisibles;
            }
            else if(nLineas>0  && lineas.get(nLineas-1).getEnd()+5 > MAXH){
                isP = false;
            }
        }else{
            isP = false;
    ...
    public String[][] consult()
    {
        int maxFosiles=0;
        isOk=true;
        String[][] consulta=new String[nLineas][0];
    ...
    {
        isOk=true;
        if(isVisible && nLineas>=1 && lineas.get(nLineas-1).getEnd()+3*15>MAXH){
            mensajeUsuario("el simulador se hará invisible por el limite de la pantalla");
            makeInvisible();
        }
        lineas.add(new Path());
        lineas.get(nLineas).move(0,(int)(Math.ceil(MAXV/3.28)+30));
        nLineas++;
    ...
    private void helpFossil(String sequence)
    {
        ...
        if(realSequence.length()>actual.getLen()){
            mensajeUsuario("No se puede agregar porque es mayor que la cadena actual del planeta");
            isOk=false;
        }
        if(isVisible && isOk && (nFosilesLibres > (Math.ceil(MAXV/3.28)-2*sz)/sz+1 || sequence.length()>(MAXH/sz)-1)){
            mensajeUsuario("el simulador se hará invisible por el limite de la pantalla");
            makeInvisible();
        }
    ...
    private void helpSituate(int fossil, int path, int position)
    {
        ...
        if(isVisible && isOk && ((fosiles.get(fossil-1).getSequence().length()>max && lineas.get(nLineas-1).getEnd()+(fosiles.get(fossil-1).getSequence().length()-max)*sz>MAXH) || lineas.get(path-1).getLength()*(sz+10)>MAXV-Math.ceil(MAXV/3.28)-sz)){
            mensajeUsuario("el simulador se hará invisible por el limite de la pantalla");
            makeInvisible();
        }
        if(isOk){
    ...
    public void situate(int fossil,int path, int position)
    {
        ...
        if(isOk){
            ...
            isOk= actual.checkSequence(tmpSequence);
            if (isOk){
    ...
    {
        if(isOk && (fossil <1 || fossil> nFosilesLibres || path < 1 || path > nLineas)){
            mensajeUsuario("Los datos ingresados sin incorrectos");
            isOk=false;
        }
        int max=0;
    ...
    /**
     * Este metodo ayuda a el situate(fossil)
     *
     * @param  fossil, el fosil a agregar
     * @param situado, si se puede situar
     * @param i, el indice a revisar
     * @param max, que es el valor de la cadena mas larga
     */
    private boolean helpSituate(int fossil, boolean situado, int i, int max)
    {
        if(isOk){
            Fossil tmp=fosiles.remove(fossil-1);
            String tmpSequence=tmp.getSequence();
            isOk= actual.checkSequence(tmpSequence);
            if (isOk){
                lineas.get(i).addFossil(tmp);
                nFosilesLibres--;
                ubicarLinea();
                situado = true;
                max=lineas.get(i).getFossil(lineas.get(i).getLength() - 1).getLength();
                if(isVisible && isOk && (((tmp.getSequence().length() > max) && lineas.get(nLineas - 1).getEnd() + (tmp.getSequence().length() - max) * sz > MAXH) 
                || lineas.get(i).getLength()*(sz+10)>MAXV-Math.ceil(MAXV/3.28)-sz)){
                    mensajeUsuario("el simulador se hará invisible por el limite de la pantalla");
                    makeInvisible();
                }
            }else{
    ...
    {
        isOk = true;
        if(isOk && (fossil <1 || fossil> nFosilesLibres || nLineas==0)){
            mensajeUsuario("Los datos ingresados sin incorrectos");
            isOk=false;
        }
        if(isOk){
            boolean situado=false;
            for (int i=0;i<nLineas && !situado;i++){
                int max=0;
                isOk = lineas.get(i).isAddable(fosiles.get(fossil-1));
                situado=helpSituate(fossil,situado,i,max);
            }
            if(!situado){
